...
export default function CommunityFeed({
  ...
  function askDelete(itemId: string) {
    ...
    if (found.kind === "post") {
      setDeleteTarget({
        id: found.post.id,
        kind: "post",
        authorName: getDisplayName(found.post.user),
        preview: found.post.content,
      });
      return;
    }
    ...
    if (found.reply) {
      setDeleteTarget({
        id: found.reply.id,
        kind: "reply",
        authorName: getDisplayName(found.reply.user),
        preview: found.reply.content,
      });
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      const response = await fetch(`/api/community/${deleteTarget.id}`, {
        method: "DELETE",
      });
      ...
      setPosts((prev) =>
        prev
          .filter((post) => post.id !== deleteTarget.id)
          .map((post) => ({
            ...post,
            replies: post.replies.filter((reply) => reply.id !== deleteTarget.id),
          }))
      );
      ...
      if (replyingToId === deleteTarget.id) {
        cancelReply();
      }
    ...
    <>
      ...
      {deleteTarget ? (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[28px] border border-white/10 bg-[#0b0f17] p-6 shadow-2xl shadow-black/50">
